...
const viewCart = async (req, res, next) => {
  ...
  const cartData = await Cart.findOne({ user_id: guestId }).populate({
    path: 'bag',
    populate: { path: 'pro_id', model: 'Product' },
  });
...
const removeCart = async (req, res) => {
  try {
    ...
    const guestId = req.session.guestId;
    const removeBag = await Cart.findOneAndUpdate(
      { user_id: guestId },
      {
        $pull: {
          bag: {
...
const doCartInvoice = async (req, res) => {
  try {
    const { totalMoney, amount } = req.body;
    const guestId = req.session.guestId;
